...
add_action('init', function() {
	if (WordpressOpsHelper::isCurrentUserAdmin()) {
		if (WordpressOpsHelper::isWPAdminPage()) {
			require_once dirname( __FILE__ ) . '/wp_admin.php';
			$wpadmin = new WPRadarWPAdmin();
			add_action('admin_menu', array($wpadmin, 'menu'));
		} else if ((array_key_exists('bvplugname', $_REQUEST)) && ($_REQUEST['bvplugname'] == "wp_radar")) {
			if (isset($_REQUEST['vuln_id']) && isset($_REQUEST['target_url']) && isset($_REQUEST['site_key'])) {
				...
				VulnerabilitySimulator::simulate($_REQUEST['vuln_id'], $_REQUEST['target_url'],
						$_REQUEST['site_key']);
			}
		}
	} else if (isset($_REQUEST['sig'])) {
